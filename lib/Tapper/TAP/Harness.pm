...
sub generate_html
{
        ...
        my $TAPVERSION = "TAP Version 13";
        my @files = map {
                         my $safe_section_name = $_->{section_name};
                         if ($_->{section_name} =~ m'[./]') {
                                 $safe_section_name = "autogenerated_filename_$safe_section_name";
                                 $safe_section_name =~ s/\./_dot_/g;
                                 $safe_section_name =~ s|/|_slash_|g;
                         }
                         my $fname          = "section/$safe_section_name";
                         my $rawtap         = $_->{raw};
                         $rawtap            = $TAPVERSION."\n".$rawtap unless $rawtap =~ /^TAP Version/msi;
